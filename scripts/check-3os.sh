...
# ★ rustup 툴체인을 쓴다(09-16 mac 실기): PATH에 Homebrew Rust(/usr/local/bin/cargo)가 ~/.cargo/bin보다 앞서면
#   `rustup target list`는 "설치됨"인데 cargo는 brew 것이라 크로스 타깃 std를 못 찾는다(E0463 "can't find crate for core").
