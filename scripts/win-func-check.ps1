...
Run-Scenario -Id S72 -Title "파일 검색 범위 필터 = 포함 *.sql + 제외 -big*(§122)" -Cmd ("view.search,@after:1000:search.run:SELECT|*.sql;-big*") -ArgList ("Local `"" + $skipDir + "`"") -WaitMs 4000 -Expect "범위 상자 '*.sql,-big*' · 결과 ok.sql 2 일치만 · bin.dat는 패턴으로 빠져 Skipped 없음(상태줄에 skipped 없음) · 상자 끝 × 표시"
Run-Scenario -Id S73 -Title "파일 검색 결과 가로 스크롤 + 열린 탭 범위 필터(§124)" -Cmd ("open:" + $qOutline + ",@after:800:view.search,@after:1200:search.run:SELECT|*.sql,@after:3200:ui.click:120/249,@after:3800:ui.hwheel:180/300/360") -ArgList ("Local `"" + $skipDir + "`"") -WaitMs 5000 -Expect "열린 탭 q_outline.sql(.sql)은 결과에 남고 Script_1(이름 없는 탭)은 *.sql에 안 맞아 빠짐 · Skipped 펼침 뒤 틸트 휠 → 행이 왼쪽으로 밀리고 아래 가로 스크롤바 · 긴 이유 글이 스크롤로 닿음"
Run-Scenario -Id S74 -Title "이진 파일 아웃라인 = 무시 + 안내(§125 · 종전 = 앱 종료)" -Cmd ("open:" + (Join-Path $skipDir "bin.dat") + ",@after:1200:view.outline,@after:1800:goto.symbol") -WaitMs 4000 -Expect "앱이 살아 있음(창 1) · 아웃라인 패널 비어 있음 · Goto Symbol 대신 상태줄 '이 파일에는 아웃라인·완성을 쓰지 않습니다…' · 편집기에 NUL 포함 본문"
