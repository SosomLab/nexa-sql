...
- **09-17 Windows 세션(52차 · [journal](docs/journal/2026-09-17.md))**: ■ 중지 버튼 결함 · **DR-33**(nsql-core `RowSource` 포트 · `ResultData` Arc 세그먼트 · `View` 인덱스 투영 · nsql-io 제네릭 · 복사/텍스트 보기/CLI 렌더러 하나 · `grid.null_text`/`cli.null_text` · 텍스트 캐시 예산) · **툴바 그룹 도크/플로팅**(nexa-ctl `ToolDock`·`ToolGroup`·`DockLayout`·`ToolItem::separator` · `toolfloat.rs` · 설정 `toolbar.layout` · View ▸ 툴바 배치 초기화 · 편집 UI = T-105) · 포터블 D-78/T-106 · **실행 속도 향상 `perf.boost`**(39 §4-6 · 강제값 > 개별 값 > 프리셋 · UI 전용 키만) · 트랜잭션 로그 설계 44(T-107 · DBMS 공용성 §4 · 실행 중지 T-108) · 툴바 트랜잭션 버튼(`TxClass` 배지·상태색) · 향상 모드 벤치 [45](docs/45-perf-boost-benchmark.md).
