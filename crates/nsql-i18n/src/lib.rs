...
impl Msg {
    ...
    const fn row(self) -> [&'static str; 2] {
        match self {
            ...
            Msg::LblButtonHoverFade => ["Button hover fade-in (ms)", "버튼 호버 강조 시간(ms)"],
            Msg::DescButtonHoverFade => [
                "A hovered button highlights gradually over this many milliseconds (faster than rows, so the button that is ready to click stands out). 0 = instant",
                "마우스가 올라간 버튼이 이 시간 동안 서서히 강조된다(행보다 빠르게 — 클릭 준비가 된 버튼 식별). 0 = 즉시",
            ],
            Msg::LblGridHoverFade => ["Row hover fade-in (ms)", "행 호버 강조 시간(ms)"],
            Msg::DescGridHoverFade => [
                "The row under the mouse (not the selected row) darkens gradually over this many milliseconds — result grid and login list alike. 0 = instant",
                "마우스가 올라간 행(선택 행 아님)이 이 시간 동안 서서히 진해진다 — 결과 그리드·로그인 목록 공통. 0 = 즉시",
            ],
    ...
    pub const ALL: &'static [Msg] = &[
        ...
        Msg::LblGridHoverFade,
        Msg::DescGridHoverFade,
        Msg::LblButtonHoverFade,
        Msg::DescButtonHoverFade,
