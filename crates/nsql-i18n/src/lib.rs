    ...
    const fn row(self) -> [&'static str; 2] {
        match self {
            ...
            Msg::DescProbeMaxRetries => [
                "After a failure retry with exponential backoff (2s, 4s, 8s… up to 60s) at most this many times",
                "실패 후 지수 증가 간격(2초, 4초, 8초… 최대 60초)으로 최대 이 횟수까지 재시도",
            ],
            Msg::LblProbeTimeout => ["Status light timeout (s)", "신호등 타임아웃(초)"],
            Msg::DescProbeTimeout => ["Per-attempt TCP connect timeout", "시도 1회의 TCP 연결 타임아웃"],
