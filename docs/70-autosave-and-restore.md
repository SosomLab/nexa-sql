...
### 6-1. 구현 현황(09-23 · 96차 후반 · journal §92)

- **저장 = 늘 지금 작업 환경을 담는다**: `project_save_to`(저장 · 새 프로젝트 · 다른 이름으로) · `project_changed(save)`(폴더 추가/제거) · 자동 저장 · 종료 — 전부 `project_capture_state()`를 지난다. 이전에는 저장 버튼 길이 상태를 담지 않고 써서 "저장했는데 탭 경로가 없다"였다.
- **열기 = 작업 환경 교체**(`project_set(p, restore=true)`는 **다른** 프로젝트를 열 때만): 복원 목록에 없던 옛 탭은 복원 **뒤에** 닫는다(깨끗한 탭 · 또는 직전 프로젝트가 있어 이미 담긴 미저장 탭). 저장·같은 프로젝트 다시 고르기는 `restore=false` — 저장을 누를 때마다 스크립트 탭이 하나씩 늘던 결함의 원인이 여기였다.
- 프로젝트 파일에 더 담는 것(§2 모델의 "좌측 패널" 부분): `preview`(미리보기 탭 표식 · 복원도 미리보기) · `expanded`(탐색기 펼침 폴더) · `panel`(보이던 좌측 패널) · `search`(검색어) · `bm_collapsed`(북마크 접힘 그룹) · `profiles`(접속 **표식만** — 복원 때 로그+토스트 "직접 접속하세요" · 재접속 없음).
- 아직 아닌 것: 편집기 **스크롤 위치**(캐럿 줄이 보이도록만) · 접힘(편집기에 접기 없음) · 저장 큐 스레드(P3 나머지).
- **09-23 정정(사용자 "미저장 탭 내용을 스냅숏과 별개로 프로젝트 파일에 탭별로 저장해 로드 때 자동 복구 · 백업은 백업용으로만")**: §5의 "참조 파일 본문은 프로젝트 파일에 쓰지 않는다"를 **프로젝트 파일에도 담는다**로 바꾼다 — 파일 탭에 미저장 편집이 있으면 `tabs[].text`(≤ 1 MB) + 그때의 디스크 해시 `hash`(글자열 · 64비트) + 탭 `id`. **복원 원천 = 프로젝트 파일**(해시가 디스크와 다르면 올리되 로그 "밖에서 바뀜") · `backups/files/`는 계속 쓰지만 **복원에 쓰지 않는다**(순수 백업 · `read_matching`은 시험 전용). 이름 없는 탭의 북마크(`Scratch { tab }`)는 저장된 `id`로 복원 때 새 id에 **재매핑**(`Bookmarks::remap_scratch`).
