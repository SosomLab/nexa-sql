...
### 10-4. 표식의 크기·자리(사용자 09-20 Mac 캡처 · journal 83차)

필수(빨강)와 바뀜(파랑) 표식은 **같은 원**이다 — 지름 `MARK_D` 6 · 글자와의 간격 `MARK_GAP` 4 · 글자 줄의 **세로 가운데**(+1px 시각 보정 — 소문자 높이 기준으로 보면 기하 중심이 위로 떠 보인다). 라벨 · 방언 콤보 · Save 버튼 · 상태줄 범례가 `connect.rs mark_dot()` 하나로 그린다(종전 = 별표 글리프와 원이 섞여 크기·기준선이 달랐다). Save 버튼의 글자는 늘 "Save" — 바뀜은 원으로만.
