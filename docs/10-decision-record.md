...
| D-125~128 | ⏳ 09-19 **대용량 파일**([59 §4](59-large-file-handling.md)): 큰 파일 모드 L1 5 MB/10만 줄 · L2 20 MB/30만 줄(권장 ①) · 버퍼 = UTF-8 갭 버퍼 + 줄 시작 표(권장 ① · D-9 로프 크레이트 안을 대체) · 되돌리기 = 개수 + 바이트 예산(권장 ③) · 큰 스크립트 = 디스크에서 실행부터(권장 ②) — **답 대기** |
