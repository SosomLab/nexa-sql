...
| 스키마 | Properties(이름) — (후속: 종류별 객체 수) | DBeaver 스키마 |
...
설명(Description)의 정의 = 객체 = 부가(`extra` · PG 서명·시노님 대상·파이프라인 …) 없으면 상태(VALID/INVALID) · 컬럼 = `타입 NULL여부` · 잎 = 부가. (후속: 테이블 코멘트를 설명으로 · `table_detail.comment`.)
...
- 스키마 = 종류별 객체 수 · 테이블 코멘트 = 설명 · 섹션 접기/탭 · 짧은 LRU · 더블클릭 = 편집기에서 열기(DDL) · 컬럼 선택 시 주인 테이블 요약 · Windows/Linux 글리프(▾/▴) 확인.
