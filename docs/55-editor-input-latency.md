...
## 09-21 추가(Windows 89차) — 편집기 밖으로 나간 드래그 선택
| 문서 | 수정 전(이동 사건당) | 수정 후 |
|---|---|---|
| 2 MB · 2만 줄 | 1.18 ms | < 0.001 ms |
| 20 MB · 20만 줄 | 12.7 ms | < 0.001 ms |

원인 = 여러 줄 드래그 처리가 마우스 이동마다 `edit.text()`로 본문 전체를 만들어 첫/마지막 줄을 판정 + 옆으로 나가면 y를 무시하고 한 글자씩 이동. 고침 = 줄 표 조회 · 옆 밖 = y의 줄의 처음/끝 · 위·아래 밖 = 거리 비례 1~12줄. 측정 = `bench_editor`의 "drag outside-left ×200". 상세 = [journal 09-21](journal/2026-09-21.md) 89차 추가.
