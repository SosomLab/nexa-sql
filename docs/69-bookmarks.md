...
**표시 이름은 열쇠에 담지 않는다**(사용자 09-23 · journal §118): 북마크는 탭 **id**로만 묶이고, 패널·토스트가 보여 주는 문서 이름은 그때그때 **id → 지금 탭 제목**(`BookmarksPanel::set_tab_titles` · `Editors::tab_titles()` · 제목 세대 `titles_rev`가 바뀐 틱에만 갱신)으로 푼다 — 탭 이름을 바꿔도 만들 때의 이름(`Script_5`)이 남지 않는다. 표에 없는 id(닫힌 탭) = `(closed tab #id)` · 접힌 문서 상태도 이름이 아니라 열쇠로 기억한다.
...
| **B4b** 🚧 09-22(우클릭 메뉴 넷 · Delete · 5초 되돌리기 토스트 · 제자리 이름) · **09-23 미리보기(한 번 클릭 = ◦ 탭) · 더블클릭/Enter/메뉴 Open = 정식 탭(승격 포함) ✅**(journal §92 · `BmAction::Goto(id, permanent)`) · 남음 = 드래그·메모 | **패널 조작** — 미리보기(좌클릭)·이동(더블클릭·Enter)·키보드 전부 · 우클릭 메뉴 넷(항목·무효·문서·그룹·빈 곳) · 제자리 이름 편집 · 드래그 이동(고스트·Esc) · 제거 + 5초 실행 취소 | B4a |
| **B4c** ✅ 09-22(순서 드래그 제외) | **그룹 관리** — 만들기·이름·기본 지정·켜기/끄기·삭제 2택·순서·상한 · 순회/거터가 `enabled`를 존중 | B4b |
| **B4d** 🚧 09-22(14키) | **설정 영역** — `GrpBookmarks` + 분류 셋 · 키 38개 등록(라벨·설명 `Msg` · 종속 · HIDDEN · `perf.boost` 강제값) · 패널 ▸ "북마크 설정 열기" | B4a |
