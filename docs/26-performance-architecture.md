...
| ★ 메타 3층 L2 워머([85 §3](85-metadata-layers.md)) | 백그라운드 메타 세션에 현재 스키마 관계 컬럼 질의 ≤ `meta.warm_columns_max`(200) | L1 완성 뒤 · `meta.warm_idle_ms`(300 ms) 간격 · 한 번에 하나 · 컬럼이 없는 것만 · 큐 소진 = 0 | 없음(실패 = 상태 되돌림) · 끄기 = `meta.warm_columns_max=0` | `Explorer::arm_warm`/`enqueue_warm_columns`/`warm_step` |
