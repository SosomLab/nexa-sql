...
| ★ 탐색기 검색 인덱스·유휴 선적재([84](84-explorer-search-index.md) · 09-25) | 메타 세션(검색 중 · prio 2) 또는 백그라운드 메타 세션(선적재)에 스키마당 카탈로그 질의 1 | 검색어가 있을 때 안 읽은 스키마를 하나씩(현재 스키마 먼저 · 진행 중 1) · 선적재 = 검색 없음 + 마지막 응답 뒤 `explorer.index_idle_ms`(5 s) + 온라인 · 스키마 전부 읽으면 0 · 접속한 적 없는 서버 0 · 상한 `explorer.index_max` | **없음**(실패 스키마 = 다음 무효화 때) · 끄기 = `explorer.search_index`/`explorer.index_prefetch` | `Explorer::ensure_index`/`pump_index`/`prefetch_step` · `nsql_catalog::name_index` |
