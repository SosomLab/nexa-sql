...
| 조각 | 자리 | 내용 |
|---|---|---|
| 값 창 | `sqlprev_win.rs` **값 모드**(`open_value` · SQL Preview 창 골격 재사용 = 창 코드 복사 0) | 제목 `Value — 열` · 보기 = **Text**(편집 가능 셀이면 편집 · "셀에 반영" = `Grid::set_cell_value` → ChangeSet · 저장은 ✓ 적용) / **Hex**(`grid::hex_dump` · `grid.lob_view_max_mb` 넘으면 앞부분만 + 안내) / **Image**(PNG(Adam7 포함)·JPEG(기저 · nexa-gfx `jpeg.rs` · §242)·BMP·GIF = nexa-gfx `image::decode` · 비율 유지 맞춤 · 안내 줄 `PNG 640×480 · n bytes` · 프로그레시브 JPEG/WebP = 판별만 + "파일로 저장해 보세요") · 버튼 = Hex · Image · 셀에 반영 · 파일에서 넣기… · 파일로 저장… · 복사 · 닫기(상황별) · 기본 파일 이름 `열_행.확장자`(형식 판별) |
...
남음 = 프로그레시브 JPEG(기저는 §242 자체 디코더 ✅ · Adam7 ✅) · CLOB 편집 상자의 큰 파일 모드(59) · 이미지 확대/축소·저장 형식 변환.
