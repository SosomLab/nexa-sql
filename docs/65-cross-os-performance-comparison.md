...
|---|---:|---:|---:|---:|---:|
| 창까지 | 141 ms | **47 ms** | (미계측) | 522 ms | **105~111 ms** |
| 3초 CPU | 500 ms | **156 ms** | — | 430 ms | **120 ms** |
| 예산 300 ms | ✅ | ✅ | — | ✗ | ✅ |
...
| 향상 모드 | 10.6 | 3.4 / 30.7 | |
| macOS 참고 | — | — | `memtrim` 148 → 56 MB(62 §3) · 큰 파일 열림 확인 · 시나리오 표는 미측정 |
...
3. ~~Windows·mac에서 아이콘 메모의 기동 효과 A/B(같은 코드).~~ ✅ **Windows 09-22 확인**(창까지 141 → **47 ms** · 3초 CPU 500 → 156 ms · §2-1) — **mac은 남아 있다**(기동 자체가 아직 미계측).
