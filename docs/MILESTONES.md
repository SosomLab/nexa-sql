...
| 🚧 | **09-22**(linux 91차): ★ Linux 첫 세션 — Oracle Instant Client 셸 환경 · 성능 전수(26 §7-7 · Windows와 같은 메모리 형상 · 릭 없음) · Linux 기동 병목 둘 수정(글꼴 트리 걷기 · 아이콘 래스터 메모/지연) 522 → **111 ms** · `[startup]` 계측 · 전체 테스트 자동화 `scripts/linux-all-tests.sh`(통합 13/13) · OS별 차이표(61 §3) · 스크립트 `scripts/linux-*.sh` 6종 |
