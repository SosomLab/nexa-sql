...
> **출처**: `SosomLab/nexa-dir2` → `nexa-beep` → `nexa-clip` `docs/16-doc-git-conventions.md`를 **2026-09-12 차용**(내용 동일).
...
9. **결정(D-)·할 일(T-) 번호는 한 줄로 늘어난다.** 번호를 잡기 전에 `git pull` → [10](10-decision-record.md)·[TODO](TODO.md)의 **최댓값 + 1**. 두 기기(Windows·맥)가 같은 구간을 쓴 것이 뒤늦게 드러나면 **기록은 고치지 않고** 표에 구간 대응을 적고, 그 구간은 문서 번호를 앞에 붙여 부른다(`56·D-103` · 09-21 D-100~110 중복 정리).
