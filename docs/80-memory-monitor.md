...
- T-197 ✅ 1차(이 문서 · `memstat.rs` · `mem_win.rs` · 배선 · 설정 4 · 위키) · ✅ 2차(09-24 §182): 풋프린트 스파크라인(최근 60 표본 · 막대 · 480 B) · **힙 정리** 버튼(`memtrim::trim` → 즉시 표본 · 상태줄 "힙 정리: 전 → 후 (ms)") · CLI `nsql mem`은 폐기(CLI 프로세스 자신의 총량만 보여 의미 없음).
