...
| 27 | ★ **그리드 데이터 편집**(T-182 · §217 · [87](87-grid-data-editing.md)) — 단일 테이블 SELECT 결과에서 Enter/더블클릭/타이핑 = 편집 · Delete = 비움 · 우클릭 = NULL/복제/추가/삭제/값 보기/미리보기/적용/되돌리기 · 툴바 `+ − ⧉ ✓ ✕` · ⌘V 행렬 붙여넣기(행 자동 추가) · ✓ = 한 트랜잭션 · 푸터 `수정 n · 추가 m · 삭제 k` | ★ **자동 E2E `scripts/mac-grid-edit-e2e.sh`**(SQLite 4 시나리오 · `NSQL_E2E_ORACLE=<접속 문자열>`이면 Oracle 임시 표 2 · 18 검사 · §226) · 실기 = 편집기 상자·붉은 띠·메뉴·변경 셀 색 |
| 28 | ★ **행 식별 등급 · 숨은 열 주입**(T-231 · §227 · [87 §13-7](87-grid-data-editing.md)) — PK 열이 빠진 결과 = 키 열 자동 주입 재조회 · 키 없는 표 = ROWID/rowid/ctid 주입(완전 중복 행도 정확히 1행) · 실패 = 원문 복귀 + 전 열 비교 · 숨은 열은 화면·복사에 없음 · 설정 `grid.edit_hidden_keys/rowid/all_cols/concurrency` | 단위 = `editable` 5(MC/DC) · `gridedit_sql` 주입·물리 WHERE·순서·동시성 4 · `grid` 재조회 경로 3 · E2E `mac-grid-edit-e2e.sh` ⑦⑧(SQLite) ⑨⑩(Oracle 임시 표) = 37 검사 · 실기 = 상태줄 "행 식별 열을 가져오는 중…" · 미리보기 머리 주석 "행 식별 = 물리 행 식별자" |
| 29 | ★ **적용 뒤 행 단위 재조회**(T-230 P1 · §228 · [87 §12-4-a](87-grid-data-editing.md)) — ✓ 적용 성공 뒤 바뀐 행만 키로 다시 읽어 제자리 교체(추가 행은 그 자리에 실제 행으로 · 삭제 행 제거) · 스크롤·정렬·선택·열 폭 유지 · 행을 식별할 수 없으면 전체 재조회 · `grid.edit_refresh = rows/requery/local` | 단위 = nsql-core `result_data_set_push_remove_rows` · gridedit_sql `refetch_stmt_cases` · grid `apply_done_rows_patches_in_place` · `delete_only_and_local_bake` · E2E 덤프 `patched=3/1/0`(①) `1/0/1`(②) `1/0/0`(⑦ · Oracle ⑥) + 제자리 값 `|KIM2|` = 42 검사 · 실기 = 적용 뒤 로그 "바뀐 행만 제자리에서 다시 읽었습니다" · 스크롤 위치 그대로 |
| 30 | 그리드 Tab/Shift+Tab(§229) · 상세 패널 코멘트 캐시 무효화 연동(T-227 후속) · 위키 [Data-Editing](wiki/Data-Editing.md) | 단위 = grid `tab_moves_selection_and_commits` · objdetail `forget_comments_by_scope` · 실기 = 탐색기 새로 고침 뒤 상세 패널 코멘트가 새 값으로 |
