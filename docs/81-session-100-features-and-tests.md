...
| 27 | ★ **그리드 데이터 편집**(T-182 · §217 · [87](87-grid-data-editing.md)) — 단일 테이블 SELECT 결과에서 Enter/더블클릭/타이핑 = 편집 · Delete = 비움 · 우클릭 = NULL/복제/추가/삭제/값 보기/미리보기/적용/되돌리기 · 툴바 `+ − ⧉ ✓ ✕` · ⌘V 행렬 붙여넣기(행 자동 추가) · ✓ = 한 트랜잭션 · 푸터 `수정 n · 추가 m · 삭제 k` | 격리 E2E: `NSQL_STARTUP_CMD="open:<select.sql>,@after:2500:run.all,@after:5500:grid.edit.set:0;1;KIM2,@after:7000:grid.edit.cmd:row.dup,@after:9000:grid.edit.cmd:row.save,@after:14000:grid.dump:<f>"` → 덤프 `Modified/Inserted/Deleted` · CLI로 서버 값 확인 · 실기 = 편집기 상자·붉은 띠·메뉴 |
