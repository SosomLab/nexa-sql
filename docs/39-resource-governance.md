...
| **객체 상세 패널**(T-223 · 86) | 없음(탐색기 메타 스레드로 `Req::Details/Comments` 1 왕복) | 그리기 = 텍스트박스 한 개(`preview_box`) · 선택마다 왕복 0(메타·테이블 단위 코멘트 캐시) | ⚠ **PK·인덱스 응답이 패널 로컬 캐시에만**(`MetaStore::set_detail` 미경유 · `MetaDetail` 원장 0 · T-227) | — | 패널 닫힘 · 대상 바뀜(LRU 없음) | `explorer.details` | — |
