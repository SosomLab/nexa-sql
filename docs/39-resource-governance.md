...
| ★ 메타 3층 L1/L2([85](85-metadata-layers.md) · [84](84-explorer-search-index.md)) | L1 접속 직후 스키마당 1 질의(250 ms 간격) · L2 현재 스키마 컬럼 ≤200(300 ms 간격) · L3 회수 30 s 점검(질의 0) | `explorer.search_index` · `explorer.index_prefetch` · `explorer.index_idle_ms` · `meta.warm_columns_max` · `meta.detail_max/detail_ttl_secs/cols_ttl_secs` | on | on | prefetch off · warm 0 | T-219·T-221 |
...
| **탐색기 검색 인덱스 L1 + 검색 판정**(T-219·T-221 · 100차 · 84·85) | 탐색기 메타 스레드(`tx_bg` 우선순위 큐) · 판정도 그 스레드(`Req::Search`→`Resp::Hits`) · 디스크 캐시 `nsql-metacache` 1회 | UI = 결과 반영만(`refilter` O(n) · 09-25 92 → ≤ 5 ms) · 애니메이션 프레임 ≤ 28 ms(26 §7-12 s10) | L1 이름 = `MetaStore` **한 벌**(28 스키마 2.4 MB · 검색 스레드 사본 `l1_names`는 세대 열쇠) · 부분 폴더 `n/?` | `NSQL_HOME/meta/<해시>.names` ≤ 1 MB(85 §9) | L3 TTL·상한(`meta.detail_max/…ttl_secs`) · 접속 해제 | `explorer.search_index` · `explorer.index_prefetch` · `meta.disk_cache` | prefetch off |
| **L2 컬럼·코멘트 워머**(T-221 · §212) | 같은 백그라운드 세션(한 번에 하나 · `meta.warm_idle_ms` 간격) | 0(도착 = `set_columns`/`set_schema_comments` 원자 교체) · 진행 중 프로세스 CPU +70 ms/초(26 §7-12 s9↔s9b) · 끝나면 0 | `Snapshot.cols`/`col_comments` **한 벌** · 패널·인텔리센스는 스냅샷을 읽는다 | 스키마당 질의 2(코멘트) · 테이블당 1(컬럼) | `cols_ttl_secs` · 접속 해제 | `meta.warm_columns_max`(0 = 끔) · `meta.warm_comments` | 둘 다 off |
| **객체 상세 패널**(T-223 · 86) | 없음(탐색기 메타 스레드로 `Req::Details/Comments` 1 왕복) | 그리기 = 텍스트박스 한 개(`preview_box`) · 선택마다 왕복 0(메타·테이블 단위 코멘트 캐시) | 서버 칸 `detail_cache`(섹션 · 열쇠 = 대상 · `MetaDetail` 원장 등재 §219) · 재선택 왕복 0 · 새로 고침 4경로 `dirty` | — | 유휴 30초 틱: TTL(`meta.detail_ttl_secs`) 미사용 제거 · `meta.detail_max` LRU · 접속 교체 = 전부 | `explorer.details` · `meta.detail_ttl_secs` · `meta.detail_max` | — |
| **그리드 데이터 편집**(T-182 · 87 · 100차) | 없음(UI 스레드 · 적용은 워커 세션 1 왕복 묶음) | 편집 상자 1개 · 덧그림 = 바뀐 셀만(`ChangeSet` BTreeMap) · 페인트 추가 비용 = 행당 상태 조회 O(log n) | 원본 세트는 그대로(DR-33) · 변경 집합 한 벌 · 복사는 선택 크기 스냅숏 | — | 적용 성공/재조회/결과 교체 시 즉시 | `grid.edit` · `grid.paste_max_rows` | — |
| **SQL Preview 모달**(T-205 · 83 §4) | 없음(생성 = 메타 스레드 1 왕복 · DBMS_METADATA) | 창 하나 = IOSurface 2벌 **+25 MB**(26 §7-12 s12) | 본문 = 편집기와 같은 상자(`preview_box` · 읽기 전용) | 파일로 저장 1회 | 창 닫힘(즉시) | — | — |
...
| **★ WASM 확장 런타임**(98차 · [75](75-extension-sdk-and-dynamic-loading.md)) | 없음(UI 스레드 · 호출마다 새 인스턴스) | 호출 = 인스턴스 생성 + 인터프리트(정책 층 µs~ms) · **페인트 경로에서 부르지 않음** · 설정 변경·명령 때만 | 모듈(검증·컴파일) 확장당 1 · 인스턴스 선형 메모리 ≤ 16 MB(호출 뒤 해제) · 모듈 파일 ≤ 8 MB · 연료 5천만 · 200 ms · 연속 실패 3 = 정지 | 설치 때 GET(파일마다 1회 · 26 §8) · 로드 때 파일 읽기 1 | 삭제 = 내림 · 관리자 끔 = 효과 0 | `extensions.enabled`(관리자) · `extensions.disabled` · (상한은 상수 — 바꿀 일이 생기면 `extensions.wasm_*` 키로) | 미등재(설치·명령에만 딸림 · 앱 크기 +wasmi ≈ 1.2 MiB는 26 §7 인벤토리에) |
| **★ 코드 완성 · 아웃라인**(98차 · [76](76-intellisense-and-outline.md)) | 없음(UI 스레드 · D-202 워커는 예산 실측 뒤) | `.`/2자+250 ms/Ctrl+Space 때만 후보 계산(스냅샷 이진 탐색 O(log n + k) + 랭킹 ≤ `intel.max_items`) · 아웃라인 = 본문 세대가 바뀐 뒤 유휴 틱에 1회(문장 분리 + 낱말 걷기) · 큰 파일 단계(L1+) 끔 | 탭별 `DocCache`(아웃라인 + 문서 단어 ≤ 5,000 · 탭 32개 넘으면 정리) · MRU 20 · 메타는 탐색기 `MetaStore`(64 MB 상한 · 컬럼 LRU) 공유 · 사본 0 | 컬럼 즉시 채움 = 탐색기 메타 세션으로 1건(`Req::ColumnsMeta` · 새 접속 0) | 접속 해제 = `meta.clear()` · `intel.enabled` 끔 = 캐시 비움 | `intel.enabled` · `intel.auto_activation` · `intel.delay_ms` · `intel.max_items` · `intel.popup_rows` · `intel.document_words` · `intel.functions`(정적 표 · 99차) · `intel.budget_ms`(후보 조립이 넘기면 로그 창 한 줄 · D-202 근거) | **`intel.auto_activation` 등재**(향상 모드 = off · 수동 Ctrl+Space만 · 타이핑마다 후보 계산 0) |
| **★ 완성 메타 버킷**(100차 mac · [76 §9](76-intellisense-and-outline.md)) | 없음(탐색기 메타 세션 스레드 재사용) | 접속 직후 현재 스키마·사전 질의 ≤ 8(함수 계열 3 포함 · `intel.from_routines`) · `스키마.` 때 ≤ 7 · 후보 조립은 스냅샷 이진 탐색 그대로 | 메타 저장소 한 벌(탐색기·완성·툴팁 공용 · `meta.cache_mb` 상한 안) · `스키마.` 버킷은 **열린 문서가 그 이름을 안 쓰면 유휴 틱에 즉시 해제**(`reclaim_intel_buckets` · 현재 스키마·사전·트리가 펼친 것은 유지 · 접속 해제 = 전부) | `intel.preload` · `intel.from_routines` | — |
| **★ 메모리 모니터**(100차 mac · [80](80-memory-monitor.md)) | 없음(타이머·스레드 0) | 창 닫힘 = 그릴 때 OS 조회 1회/5 s(≈ µs) · 창 열림 = `mem.refresh_ms`(1 s)마다 부품 순회(편집기 탭·결과 탭·서버 칸 · O(1) 어림) + 창 1회 그리기 | `Sample` 1개 · 창 표면(닫으면 해제) · 아이콘 캐시는 `Intel`/탐색기 것을 읽기만 | `mem.statusbar`(끄면 조회 0) · `mem.refresh_ms` · `mem.status_refresh_ms` | — |
...
| **대량 적재 `nsql import`**(09-26 · 89) | 스레드 0(CLI 주 스레드) · 원료 스트리밍(`DelimReader` 64 KB 버퍼 · 파일 전체 안 듦) · 배치 버퍼 = `bulk.batch_rows` 행 · Oracle 배열 버퍼 2× 배치 | `bulk.batch_rows` · `bulk.commit_every` · `bulk.mode`(single = 최소 메모리) | 없음 | 끝나면 전부 | (CLI) | 26 §8 |
