...
| S-11 | GFX | 다시 그리기는 **변경이 있을 때만**(입력·이벤트·애니메이션 목표 변경) · 마우스 이동만으로 전체 창 재그리기 금지(hover는 `IntentFade` 70 ms 의도 판정 뒤) · 툴팁·페이드·깜빡임 각각 끌 수 있다 | 프레임 카운터(T-90e `--trace-frames`: 초당 그린 횟수·영역) | 규칙 ✅ · 카운터 없음 |
