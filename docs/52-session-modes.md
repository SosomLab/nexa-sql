...
| 화면 | 한 번에 한 서버의 트리 — **활성 탭의 서버를 따라간다**(전환 = 포인터 교체 · 재접속/재조회 0) · 서버가 둘 이상이면 머리줄 `이름  i/n ▾` → 서버 목록(● 지금 · 오프라인 표시 · 제거) | 탭마다 다른 서버여도 맞는 메타 |
...
⑯ 서버 A·B 공유 연결 → 탐색기 머리줄 `이름 1/2 ▾` · 탭을 바꾸면 그 탭 서버의 트리로(재조회 없음) · A를 해제해도 A에 전용 세션 탭이 있으면 A 탐색기 유지 · 마지막 세션까지 끊으면 "오프라인"(트리는 남음) · 머리줄 메뉴에서 제거
