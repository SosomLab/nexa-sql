...
## 코드 완성과 문서 크기(09-24 §187 · 사용자 "파일 용량별 레벨에서 제한되도록")

| 단계 | 조건 | 완성 동작 | 키 |
|---|---|---|---|
| 0 | 문서 ≤ `intel.max_doc_kb`(1024 KB) | 전부(문서 전체 문장 분할 · 문서 낱말·아웃라인 캐시) | `intel.*` |
| 창 방식 | 문서 > `intel.max_doc_kb` | 캐럿 앞뒤 256 KB 창(빈 줄 경계)만 문맥으로 · 문서 낱말·아웃라인 캐시 없음 · 메타·키워드·함수·카드는 그대로 | `intel.max_doc_kb` |
| L1 | `file.large_l1_*`(5 MB · 10만 줄) | 자동 팝업·아웃라인·Goto Symbol 끔 · **수동 ⌃Space·시그니처 도움은 창 방식으로** | `file.large_l1_mb/lines` |
| L2 | `file.large_l2_*`(20 MB) | 전부 끔 | `file.large_l2_mb/lines` |
| 자기 감속 | 요청이 `intel.budget_ms`(30) 초과 연속 2회 | 그 탭의 문서 낱말 끔 · 상태줄 한 번 | `intel.budget_ms` |

실측(Release · 문서 전체 분할): 1 MB 30 ms · 4 MB 130 ms → 문턱 1 MB.
