...
## 9. 두 번째 실행(2026-09-22 · 95차 · 사용자 "누락 없이 · 정확하게 · 충분한 시간")

전 단계 A~F(5회·8초) + **C-2 R1~R6 첫 실행**(`scripts/win-mem-reclaim.ps1`) — 결과 = [26 §7-9](26-performance-architecture.md) · 회귀 0 · 회수 6항목 허용치 안. 기능 점검 자동화(`scripts/win-func-check.ps1` · journal 09-22 §62)는 성능 측정과 **순차**로(앱 인스턴스가 겹치면 유휴 CPU가 오염된다).

C-2 실행기가 배운 것 → §C-2 절차에 반영: ① `grid.max_rows`를 올리지 않으면 10만 행이 200행으로 잘려 회수 시험이 헛돈다 ② 65 MB는 `file.large_ask_mb`(기본 50) 때문에 열기 선택 팔레트가 떠 기동 명령으로는 못 고른다 → 격리 설정 0 ③ 결과 탭 닫기는 `ui.click`으로 × 자리(활성 탭이 첫 자리일 때 같은 좌표) ④ memtrim 유휴 주기는 `mem.trim_secs`로 앞당긴다(기본 300초) ⑤ R5는 실서버 — 스크립트 밖.

**이번에도 새로 재지 않은 차원**: D10 I/O 속도(84차 실측 유지) · G 병목(회귀가 없어 안 들어감 · 70만 줄 첫 페인트만 단독 재측정으로 판정).

## 10. 세 번째 실행(2026-09-23 · 96차 · 사용자 "모든 변경 정리 뒤 전체 성능 점검 다시 · 개선 확인 · 필요하면 수정 · 전체 테스트 자동화")

전 단계 A~F(5회·8초) → **회귀 1건 발견 → 수정 → C·D·E 재측정 → E 연장 시험(L2 30주기)** → 기능 점검 자동화 전수(`win-func-check.ps1` S01~S51) 순으로 돌았다. 결과 = [26 §7-10](26-performance-architecture.md).

이번 실행이 프로세스에 더한 것:
- **G 단계의 실제 사용례**: 시나리오 표에서 한 행만 튀면(프로젝트 패널 +2 MB · 핸들 +82) **D 단계의 강제 키 목록이 곧 원인 후보표**다 — 향상 모드 on 열이 이전 값으로 돌아오면 그 시나리오에서 강제되는 키(`project.icons`) 하나가 원인. 그다음 구성 요소를 나눠(워커/COM · OS 이미지 리스트 · 앱 캐시) 각각 "앱이 줄일 수 있는가"를 판정하고, 줄일 수 없는 몫은 [39 §3](39-resource-governance.md) 원장에 **수치와 함께** 등재한다.
