...
| **T-107** | P1 | 중 | ✅ 09-17 52차 — **트랜잭션 로그 창**([44](44-transaction-log.md)): nsql-run `txlog`(TxEntry/TxRecord/TxLog 링·필터·`outcome_of` 표시 시점) · `txlog_win.rs` 모덜리스(검색 · 7열 · Tx 열 = 대기/커밋됨/롤백됨/암묵/자동/끊김 · 스위치 3 · 취소선) · 호스트 `tx_close(outcome)` · `txlog.max_entries`. **잔여(T-107b)**: `tx_pending` 원천을 TxLog로 교체 · 우클릭(복사·편집기로) · 중지(T-108) 연결 · 서버 롤백 판정(44 §4) | T-108 34 39 | ✅ |
| **T-108** | P1 | 대 | **실행 중 상태·중지**(사용자 09-17): ✅ 09-17 ① `Session::cancel_handle` 포트 + SQLite/PG/Oracle 구현 + 워커 `cancel_run` + 툴바/카드 ■ + "중지됨" 판정 · ✅ 배지 · ✅ 실행 상태 카드. **잔여**: 편집기 탭 실행 중 표시 · 중지 시점까지 받은 행 표시(행 단위 · `ExecResult.interrupted` · 지금은 결과 비움) · ✅ MSSQL 취소(`mssql.encrypt=login` = Attention 세션 유지 · `required` = 소켓 종료+재접속) · 후속 = tiberius 포크로 TLS 안 Attention · 트랜잭션 로그 중지 결과 연결 · 카드 라이브 단계 이벤트 | 26 43 44 | 🚧 |
...
| **T-110** | P1 | 중 | **미니맵 표시 확장**([46](46-minimap-features.md) · 사용자 09-17): 1순위 = 찾기 결과 띠 · 오류 줄 마크 · 뷰포트 always/hover+테두리 · 클릭 center/text → 2순위 = View 토글·키맵 `view.minimap` · 현재 문장 범위 · git 변경 줄 · 수정 줄 · 좁은 창 숨김 → 3순위 = 자동 숨김 페이드 · 배율 · 왼쪽 · 섹션 헤더. 구현 = nexa-ctl `MinimapMark` 한 벌 + `set_minimap_marks(kind, ranges)` | 46 39 | 📐 |
...
| **T-115** | P2 | 소 | **Auto Indent 후속**([49 §5](49-auto-indent.md)): `bracketIndentNextLinePattern`/`unIndentedLinePattern`(주석 줄) · 문자열/주석 안 괄호 무시(강조 토큰) · 붙여넣기 재정렬(`reindent`) · `.tmPreferences` 읽기 · 다중 캐럿 증가 규칙 | 49 | ☐ |
| **T-113** | P2 | 중 | **로그 후속**([48 §5](48-logging-architecture.md)): 완전 지연 포맷(`LogRec` 32B · 텍스트는 표시 시점) · feature `devlog` 컴파일 타임 제거 · `trace` 수준 생산 지점 · `tx`/`meta` 층 · 텍스트 보기 변환 시간 | 48 39 | ☐ |
| **T-114** | P2 | 소 | **Sublime 커서 후속**([29 §7-1](29-editor-syntax-palette-statusbar.md)): Ctrl+M 괄호 짝 · Ctrl+Shift+M 괄호 안 확장 · Ctrl+Shift+Space 스코프 확장 · Ctrl+↑/↓ 스크롤만 · mac ⌘↑/↓ · 가운데 버튼 열 선택 | 29 | ☐ |
