...
| **T-148** | P2 | 소 | **DDL 점검의 잔여**([62 §5-2](62-macos-input-and-present.md)): `SHOW ERRORS` 미지원(예제 `oracle-refcursor-pkg.sql`도 쓴다 — `USER_ERRORS` 조회로 구현) · `sqlite:////abs` 슬래시 4개 = 상대 경로로 풀림(`trim_start_matches("//")` → 한 번만) · 접속 전 SQLite 오류가 `[ORA-…]`로 표기 · 시작 인자 접속의 탐색기 루트 이름 = 프로필 이름으로 · `NSQL_TRACE_MEM` 맥 측정값(`phys_footprint`) · MySQL `DELIMITER`(드라이버 도입 때) | 62 | ☐ |
| **T-147** | P1 | 중 | **맥 화면 내보내기**([62 §2](62-macos-input-and-present.md) · D-133): present 37 ms(프레임마다 21 MB CPU 색 변환) → 자체 IOSurface 내보내기 또는 softbuffer 색 공간 패치 · 프레임버퍼 2~3벌(footprint 56 MB 중 ≈ 45 MB) · 큰 파일 적재 피크 286 MB — T-136 D6(버퍼 보존)과 한 작업 | 62 55 | ☐ |
