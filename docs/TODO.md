...
| **T-48** | P0 | 대 | ✅ 09-15 1차 상한 = `grid.max_rows` 200(Oracle/SQLite 조기 중단 · `RunEvent::ResultSet.more` · `--max-rows`) · **설계 ✅ 09-16 [43](43-fetch-model-and-result-tabs.md)(D-68~72)** · **✅ 09-16 25차 1차**: OFFSET 폴백(`nsql_io::paging` · `Runner::query_once` · `Cmd::FetchPage/Count`) · 결과 도구줄 5그룹 · 보기 모드 7종 · 스크롤 끝 자동 페치 · 전체/건수 · No Records · 렌더/메모리 로그 · 잔여 = **a** 포트(`Session::fetch_next/close_cursor` · `ExecResult.pending` · 러너 fetch_next/fetch_all/count · Navigate 스팬 · Oracle/SQLite 커서 유지 · PG Portal · MSSQL OFFSET 폴백 · OFFSET 래퍼 4방언) · **b** GUI 도구줄(세그먼트 상자 · Fetch next/all(진행·취소) · Count · Refresh · 이동 · 탭 로컬 max_rows · 예산 `grid.memory_budget_mb` · 설정 8종) · **c** 스크롤 자동 페치 · tiberius 스트림 유지 · `db.cursor_idle_secs` · **d** CLI `cli.max_rows` · `\more/\all/\count` · `\pager` · `cli.auto_more` · 실행 히스토리 패널 | 43 D-68~72 | 🚧 |
...
| **T-93** | P1 | 중 | **결과 탭**([43 §4](43-fetch-model-and-result-tabs.md) D-71) — `ResultPanel{tabs}`(편집기 탭 쌍 유지) · Ctrl+Enter 교체 · **Ctrl+\\ 새 결과 탭**(자기 상한·커서·상태) · TabBar 재사용 · 우클릭(닫기/다른 탭 닫기/고정/이름/Export) · 상한 `grid.result_tabs_max` 8 + pin · 닫으면 rows·커서 즉시 해제 · 활성 탭만 그림 | T-48a T-48b | ☐ |
