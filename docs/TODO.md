...
| **T-141** | P1 | 중 | **큰 파일 모드**([59 §4 1단계](59-large-file-handling.md) · D-125): L1 5 MB/10만 줄 · L2 20 MB/30만 줄 · 기능별 축소(미니맵 표본 · 선택어 강조 · 괄호 표 범위 · 기준선 = 줄 해시 · 구문/줄 접기/자동 병합 끔) · 상태줄 표식 + 토글 + "강제로 켜기" · 저장 기준 `String` 제거 | D-125 | ☐ |
| **T-142** | P1 | 대 | **편집 버퍼 교체**([59 §4 2단계](59-large-file-handling.md) · D-126·127): `Vec<char>` → UTF-8 갭 버퍼 + 줄 시작 표(포트 `slice/line_start/line_of/replace` 뒤에) · 3보(포트 도입 → 구현 교체 → 호출부 이관) · 되돌리기 = 연산 기록 + 바이트 예산 · 기대: 3.6 MB 파일 24 MB → ≈ 4 MB | D-126 D-127 | ☐ |
| **T-143** | P2 | 중 | **큰 SQL 스크립트 관리**([59 §4 3단계](59-large-file-handling.md) · D-128): ① 디스크에서 바로 실행(스트리밍 문장 분할기 · 진행·취소 · N문장 넘으면 페치 끔) ② 열기 대화상자(읽기 전용 · 일부 보기 · 열지 않고 실행) ③ 비동기 적재 ④ 표본 인코딩 판정 ⑤ 읽기 전용 tail | D-128 | ☐ |
