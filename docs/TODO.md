...
| **T-149** | P0 | 대 | ★ **변수 관리**([63](63-variable-management.md) · 사용자 09-21 — Oracle 관용 그대로 + DBMS 무관 추상 구조 · 기준 준수 · 개발 뒤 전수 점검). ✅ V0 기반 결함(저장 코드 DDL 바인드 · 암묵 결과 전부 · 1행 흡수 방언 제한) · ✅ V1 REF CURSOR 자동 표시(`run.cursor_autoshow`) · 결과 라벨 · 서명 추론(`call_shape` + `routine_args`) · GUI 딸린 결과 탭 — Oracle 실기. ☐ V2 보이기(`VarsChanged` · 변수 패널 · 로그 값 · hover) · ☐ V3 입력 창(실행당 한 번 · `ACCEPT` · NEW_VALUE — T-16c 흡수) · ☐ V4 범위·보존(D-135·136) · ☐ V5 능력표 `Caps` · `captures` · PG refcursor · `@@ROWCOUNT` · 타입 추가 · ☐ V6 MacroStore · 시스템 변수 · `${v:형식}` · ☐ V7 전수 점검(`bench_vars` · 4방언 통합 · Codespaces). 결정 대기 D-135~138 | 63 08 | 🚧 |
