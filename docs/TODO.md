...
| **T-139** | P1 | 중 | **macOS 한글 입력**(사용자 09-19 · **Mac 세션에서 진행** — Windows에서는 재현·검증 불가): ① 편집기에서 자모가 풀리는 것 ② "한글1234" → "한글234"(한글 뒤 첫 ASCII 글자 유실 · nexa-clip/nexa-beep에서 고친 H-26 유형 · nexa-beep docs/34 `leak_intercept`/`route_char` 참고) ③ 검색 상자(찾기·파일 검색·설정·확장 패널·팔레트)도 같은 경로로 ④ 편집기 입력 속도에 영향 없어야(55 계측으로 전후 비교). 탐색기 타입어헤드는 이미 첫 자모 깨짐 없이 즉시 반영 — 그 경로가 참고 구현 | — | ☐ |
