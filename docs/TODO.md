...
| **T-149** | P0 | 대 | ★ **변수 관리**([63](63-variable-management.md) · 사용자 09-21). ✅ V0 기반 결함 · ✅ V1 REF CURSOR 자동 표시·결과 라벨·서명 추론·딸린 결과 탭 · ✅ D-138 문장마다 결과 탭 · ✅ D-135 계층형 표(`VarStore` 세 층 · `RunEvent::Vars` · `App.tab_vars` · 로그에 값) · ✅ V2-lite `SHOW VARIABLES` 표 · ✅ D-137 입력 창(`missing_inputs` · `input_win.rs` · `vars.undeclared`) · ✅ D-136 파일별 보존(`varsfile.rs` · 스크립트로 내보내기). ☐ V2 옆 패널(제자리 편집 · 공유 토글 · hover 값 · 미정의 경고) · ☐ `ACCEPT [HIDE]` · `COLUMN … NEW_VALUE` · ☐ 이름 없는 탭 보존(S-1과 함께) · ☐ V5 능력표 `Caps` · `captures` · PG refcursor · `@@ROWCOUNT` · 타입 추가 · PG 배열 조각 오탐 · ☐ V6 MacroStore · 시스템 변수 · `${v:형식}` · ☐ V7 전수 점검(`bench_vars` · 4방언 통합 · Codespaces) | 63 08 | 🚧 |
