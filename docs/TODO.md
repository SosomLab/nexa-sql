...
| **T-132** | P1 | 중 | 비밀번호를 접속 문자열에 쓰지 않는 길([52 §4](52-session-modes.md) · 09-19 자격 빌리기 제거의 후속): ① 편집기 변수(`DEFINE`/`:pw` · 값은 세션 메모리만 · 로그/이력에 남기지 않음) ② `CONNECT`에 비밀번호가 없으면 **모달 비밀번호 입력**(잃는 순간 규칙과 같은 팝업 · 저장 안 함) ③ CLI `--password-stdin`/프롬프트 · 로그인 창 세션 비밀번호(`session_pw`)와 한 저장소 | 52 §4 | ☐ |
...
| **T-150** | P1 | 중 | **PostgreSQL refcursor → 결과 탭**([63 §3-1](63-variable-management.md) · [61 §6](61-core-design-and-working-rules.md) 순서 2): 커서 이름을 같은 트랜잭션에서 `FETCH ALL IN "이름"` → `result_sets` → `CLOSE` · 열 타입(`refcursor`) 확보가 선행(단순 질의 경로에는 타입이 없다) · 자동 커밋이면 감싼다 · 검증 = `integration.rs` + 작업 브랜치에서 `gh workflow run integration.yml --ref` | 63 | ☐ |
| **T-151** | P1 | 중 | **서명 추론·타입 넓히기**: `routine_args` PG/SQL Server/MySQL · DATE/TIMESTAMP/BOOLEAN 실제 바인드(Oracle 드라이버의 VARCHAR2(4000) 일괄 바인드 걷기) · `Direction::Out` 생성 · `VAR X NUMBER`의 표기 보존 · `VAR X`(타입 없음) | 63 | ☐ |
| **T-152** | P1 | 대 | **드라이버 능력표 `Caps` 포트**([63 §3](63-variable-management.md)): 엔진·러너의 방언 `match`를 능력 질의로(marker · out_values · cursor_out · multi_result · bind_scope …) — MySQL/MariaDB·NoSQL 어댑터의 전제 | 63 30 | ☐ |
| **T-153** | P2 | 중 | **변수 UX 잔여**: `ACCEPT [HIDE] [DEFAULT]` · `COLUMN … NEW_VALUE` · 입력 창 타입 열·미리보기 · MacroStore 분리 + 시스템 변수 · `${v:형식}` · CLI `-v` · hover 값 · 미정의 경고(`Diagnostic` 소비) · `vars.max_value_kb` · 프로필 층 출처 · 이름 없는 탭 보존(S-1) · `.devcontainer`의 `NSQL_POSTGRES_URL` → `NSQL_PG_URL` | 63 | ☐ |
| **T-154** | P1 | 소 | ⏳ **Windows 실기(맥 86~88차분)**([61 §3](61-core-design-and-working-rules.md) "윈도우에서 처음 할 일"): 창 11곳 열림·그리기(`present.rs`) · 변수 입력 창·변수 창(포커스·마우스 라우팅·Windows 한글 IME) · 수동 커밋 Commit/Rollback(SQL Server·PG — T-146) · 문장마다 결과 탭·새로고침 · Oracle REF CURSOR 자동 표시 | 61 63 | ☐ |
| **T-162** | P2 | 소 | **변수 샘플 검증에서 나온 흠**(09-21 89차 추가 6 · `examples/variables/*.sql`): ✅ SQL Server `EXEC :V := 서버 식`이 오류 없이 빈 값(`SET @V`를 세션 옵션으로 오인 → `needs_declare_prepend`) 수정. **남음**: ① SQLite의 `@x` · `$x` 표기가 변수로 묶이지 않는다(`Wrong number of parameters` — [63 §3-1](63-variable-management.md)의 "한 변수"는 설계만) ② SQL Server `EXEC SELECT 식 INTO :V`(**FROM 없음**)가 구문 오류(Msg 102 — `rewrite_select_into_tsql`) ③ CLI의 대입 메아리(`:V_API_TOKEN = abc123`)가 **비밀 이름 규칙(D-140)을 안 거친다**(`SHOW VARIABLES`는 가림) ④ `DEFINE v = O'Neil`(맨 작은따옴표)이 뒤 문장들을 글자 상수로 삼킨다 → 큰따옴표로 감싸야 함(안내 또는 DEFINE 줄은 줄 끝에서 끊기) ⑤ SQL Server에서 선언 없는 변수가 받은 수가 `VARCHAR2`로 남는다(`V_CNT VARCHAR2(3) 797`) ⑥ `nsql plan -d postgres`는 서버 식 대입 뒤의 `PRINT`를 "변수 없음"으로 센다(접속 없는 plan의 한계 — 오류 대신 안내로) | 63 | 🔧 |
