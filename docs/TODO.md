...
| **T-146** | P1 | 소 | 🔧 **CI `integration` 실패**(09-19 78~79차 push부터 3회 연속 · 84차 push 뒤 확인): `nsql-drivers/tests/integration.rs` `pg_read_only_transaction_ends_in_manual_mode` 224행 — 수동 커밋에서 `INSERT; SELECT` 뒤 PostgreSQL 세션이 `idle in transaction`이어야 하는데 `idle`. 테스트 준비(테이블 생성 순서·오류로 인한 롤백)의 문제인지 **T-137 L1 읽기 트랜잭션 자동 종료가 변경이 있는 트랜잭션까지 닫는 결함**인지 가려야 한다(후자면 데이터 안전 문제) · `ci` 워크플로(빌드·단위 테스트·clippy)는 통과 |
