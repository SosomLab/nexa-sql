...
| **T-138** | P1 | 중 | **객체 탐색기 갱신**([57](57-explorer-refresh-after-ddl.md) · D-106~110 ✅ 09-19): ① `ddl_target` 파서 ② 디프 갱신(`refresh_diff` · 펼침·선택·스크롤 보존 · 2초 강조) ③ T1 실행한 DDL → 그 폴더(커밋 시점 · 스크립트 끝 1회 · 전 세션) ④ T3 루트/Shift+F5 범위 ⑤ T4 못 찾음 신호 ⑥ T2 유휴 워터마크 300초 ⑦ 설정 `meta.refresh_*` + 옛 `explorer.auto_refresh/refresh_secs` 이관 | 57 | ☐ |
| **T-137** | P1 | 중 | **수동 커밋 잠금 방지**([56 §9](56-manual-commit-lock-prevention.md) · ✅ 09-19 L1~L4 1차 · 남은 것 = ⑤ 접속 유형 프리셋 · 트랜잭션 로그 창 "차단 중" 띠 · 39 §3/26 §8 등재 · 실기): ① L1 읽기 트랜잭션 자동 종료(`tx.read_end`) ② L2 유휴 미커밋 경고·재알림·카운트다운 자동 롤백 ③ L3 막힘 감지(메타 세션 · 4방언) ④ L4 서버 안전망 세션 파라미터 ⑤ (2차) 접속 유형 프리셋 | 56 | 🚧 |
| **T-136** | P1 | 중 | **편집기 입력 지연 2차**([55 §5](55-editor-input-latency.md)): ✅ 깜빡임 위상 입력 기준(09-19 win · §6) · D6 픽셀 버퍼 보존(softbuffer 프레임당 20 MB 할당 제거) → D5 부분 무효화(`Invalidations` 합집합·층 건너뜀) · D12 휠 뒤 60 fps 핀 해제 · D16 자동 반복 합치기 · D7 하이라이터 줄 캐시 · D9/D10 글꼴 경로·아틀라스 · 39 §6 게이트(2 MB 페인트 ≤ 20 ms) | 55 39 | ☐ |
