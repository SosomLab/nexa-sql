...
- FROM 자리: 테이블·뷰·구체화 뷰·시노님이 **한 층**에서 이름·일치로 경쟁하고, 함수·패키지·프로시저는 그 아래 층(테이블 함수 먼저 · 설정 `intel.from_routines`). 함수를 고르면 `NAME()` 안에 캐럿. `스키마.테이블.`까지 치면 FROM 자리에서는 팝업이 뜨지 않습니다(테이블 수준까지). 항목 왼쪽 아이콘 = 탐색기와 같은 종류 도형(`intel.icons`).
- 강조: 친 글자와 **전체가 같은 항목은 굵은 파랑**, 부분 일치는 **일치한 글자만 강조색**(`M4S_` = 접두 4글자 · `MI40` = 약어 글자 4개).
- 순서(접두를 쳤을 때): 정확 > 접두 > 단어 경계(`_` 뒤) > 포함 > 약어 > 부분열(연속 일치·낱말 시작이 많을수록 앞). 같은 등급이면 **최근에 확정한 것** → 출처 → 아래 둘러보기 순서. 컬럼 목록 조각은 늘 맨 위.
