...
편집기 왼쪽 **북마크 영역(색 띠·니모닉 상자)이나 줄 번호**를 우클릭하면 그 줄의 메뉴가 뜹니다 — 북마크가 없으면 **토글(추가)**, 있으면 **제거**가 활성이고, **니모닉 ▸ 1~9**(없으면 만들어서 지정 · 지금 것은 ✓)와 **니모닉 해제**(니모닉이 있을 때만)가 있습니다. 본문 우클릭은 종전대로 편집 메뉴입니다.
