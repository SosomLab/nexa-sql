...
남음 = B-3 PG `COPY TO` 추출 · GUI Import 창(열 매핑·진행 카드·취소) · B-4 나머지(TVP · 파이프라이닝 · MySQL `LOAD DATA`/ODBC 배열 · SQL Server bulk 옵션 `CHECK_CONSTRAINTS`/`FIRE_TRIGGERS`는 tiberius 0.12에 API 없음).
