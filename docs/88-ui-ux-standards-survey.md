...
#### 4-1-b. 다른 메뉴의 개수·깊이(코드 수준 · 09-26 §230 보충)

| 메뉴 | 코드상 항목 | 구분선/그룹 | 서브메뉴 | 판정 |
|---|---|---|---|---|
| 탐색기 우클릭(`explorer.rs`) | 19(노드 종류별로 갈라져 한 번에 다 보이지는 않음 · 실제 ≤ 12) | 5 / 6 | 1단(Refresh ▸ 계층형) | 15·7·1단 ✓(한 노드 기준) — 실기 캡처로 노드별 수 확인 남음 |
| 편집기 탭 메뉴(`editors.rs open_tab_menu`) | 8 | — | 없음 | ✓ |
