...
#### 다른 CLI 별칭 · 스크립트 포함 · SPOOL (T-52 · T-9 · 09-16)

psql·sqlite3·sqlcmd 사용자가 습관대로 쳐도 SQL\*Plus식 명령으로 대응된다 — `\d` `\dt` `.tables` → `SHOW TABLES`(`SHOW VIEWS`·`SHOW <kind>`도 같음 · `nsql cat`과 같은 컬럼) · `SHOW ERRORS [종류 이름]` = 마지막으로 만든 저장 단위의 컴파일 오류(Oracle `ALL_ERRORS` · 없으면 `No errors.` · 09-21) · `\d <이름>` `.schema <이름>` → `DESC <이름>` · `\i <파일>` `:r <파일>` `.read <파일>` → `@<파일>` · `\c <대상>` → `CONNECT` · `\?`/`help` → 명령·별칭 표. `@파일 인자…`는 현재 폴더 기준, `@@파일`은 **호출한 스크립트의 폴더** 기준(중첩도 각자 폴더 · 확장자 없으면 `.sql` 보완)이며 인자는 `&1..&n`으로 들어가고 돌아오면 호출자의 값이 복원된다. `SPOOL <파일> [APPEND|CREATE]`은 이후 찍히는 모든 출력(현재 형식의 결과 표·메시지·`ERROR` 줄)을 파일에도 남기고(확장자 없으면 `.lst` · 시작은 조용히), `SPOOL OFF`로 닫으며 맨몸 `SPOOL`은 상태를 알려 준다 — `run`·`shell` 공통. ⚠️ SQL\*Plus처럼 `SPOOL out`은 `SPOOL OUT`(=OFF)으로 읽히니 파일 이름은 `out.lst`처럼 확장자를 붙인다. 배치에서 미정의 `&var`·선언 없는 `:bind`를 오류로 막으려면 `nsql config set script.strict on`(비노출 설정 · `config list all`).

```sh
printf 'SPOOL out.lst\nSELECT 1 AS n;\n@@sub/child.sql 42\nSPOOL OFF\n' > main.sql
nsql run -c sqlite:t.db -d sqlite main.sql      # out.lst에 표·메시지가 그대로
printf '\\d\n\\d emp\n\\i main.sql\nquit\n' | nsql shell -c sqlite:t.db -d sqlite
```
